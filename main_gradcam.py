...
import cv2
# 数据集类别名
names = ['pedestrian', 'people', 'bicycle', 'car', 'van', 'truck', 'tricycle', 'awning-tricycle', 'bus', 'motor']  # class names
# yolov5s网络中的三个detect层
target_layers = ['model_41_cv3_act']

# Arguments
parser = argparse.ArgumentParser()
parser.add_argument('--model-path', type=str, default="runs/train/exp(TOP HEAD SIOU) 44.7/weights/best.pt", help='Path to the model')
parser.add_argument('--img-path', type=str, default='runs/detect/exp(bifpn)3/0000103_03349_d_0000031.jpg', help='input image path')
...
def main(img_path):
    ...
    for target_layer in target_layers:
        ...
        for i, mask in enumerate(masks):
            ...
            res_img = plot_one_box(bbox, res_img, label=None, color=colors[int(names.index(cls_name))],#修改
                                   line_thickness=1)
            # 缩放到原图片大小
