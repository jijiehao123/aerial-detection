...
Usage:
    ...
    $ python path/to/train.py --data coco128.yaml --weights '' --cfg yolov5s-0.yaml --img 640  # from scratch
"""
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=ROOT /'weights/yolov5s.pt', help='initial weights path')#选择预训练权重模型，若default为空则用程序自带初始权重
    parser.add_argument('--cfg', type=str, default='models/yolov5-PFFN.yaml', help='model.yaml path')#选择神经网络模型
    parser.add_argument('--data', type=str, default=ROOT / 'data/VisDrone.yaml', help='dataset.yaml path')#样本数据路径，进入到mydata.yaml文件里修改修改
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/hyp.scratch-low.yaml', help='hyperparameters path')#超参数：动量、衰减率等。一般用不到
    parser.add_argument('--epochs', type=int, default=300)#训练几轮，设定epoch数量
    parser.add_argument('--batch-size', type=int, default=8, help='total batch size for all GPUs, -1 for autobatch') #每批次的数据量，default=-1时自动调节大小
    ...
    parser.add_argument('--noautoanchor', action='store_true', help='disable AutoAnchor')#用于设置在目标检测任务中是否采用锚点 / 锚框。
    ...
    parser.add_argument('--device', default='7', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')#用于设置运行 pytorch 框架的使用设备，是用 GPU cuda，还是 cpu
    ...
    parser.add_argument('--optimizer', type=str, choices=['SGD', 'Adam', 'AdamW'], default='SGD', help='optimizer')#优化器：填入到 Edit Configuration --> Parameters
    parser.add_argument('--sync-bn', action='store_true', help='use SyncBatchNorm, only available in DDP mode')#进行多 GPU 进行分布式训练。
    parser.add_argument('--workers', type=int, default=32, help='max dataloader workers (per RANK in DDP mode)')#多线程：这里建议 default 设置为 0。
    parser.add_argument('--project', default=ROOT / 'runs/train', help='save to project/name')#用于指定训练好的模型的保存路径。
    parser.add_argument('--name', default='exp(TOP_up_1_3head)', help='save to project/name')# 用于设定保存的模型文件名。
    ...
    parser.add_argument('--freeze', nargs='+', type=int, default=[0], help='Freeze layers: backbone=10, first3=0 1 2') #冻结层数设置
    ...
    parser.add_argument('--local_rank', type=int, default=[0,1,2,3,4,5,6,7], help='DDP parameter, do not modify') #DistributedDataParallel 单机多卡训练，一般不改动。
