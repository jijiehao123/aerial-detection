...
def run(
        ...
        name='exp(bifpn)',  # save to project/name
    ...
    if not training:
        ...
        task = task if task in ('train', 'val', 'test') else 'val'  # path to train/val/test PEGImages
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/exp(TOP_up_1_3head)/weights/best.pt', help='model.pt path(s)')
